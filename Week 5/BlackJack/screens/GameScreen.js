...
function GameScreen(props) {
  ...
  function drawUserCardHandler() {
    ...
    //Calculate the new user score to see if the card will make the player bust
    if (Cards[userCard].value + userScore > 21) {
      ...
      if (userHand.includes("aceClubs")) {
        ...
        //Change aceSpades to lowAceSpadess
        setUserHand((prevUserCards) => {
          let newUserCards = prevUserCards;
          newUserCards[newUserCards.indexOf("aceSpadess")] = "lowAceSpades";
          return newUserCards;
        });
  ...
  function drawComputerCardHandler() {
    ...
    //Calculate the new computer score to see if the card will make the computer bust
    if (Cards[computerCard].value + computerScore > 21) {
      ...
      if (computerHand.includes("aceClubs")) {
        ...
        setComputerHand((prevComputerCards) => {
          let newComputerCards = prevComputerCards;
          newComputerCards[newComputerCards.indexOf("aceSpadess")] =
            "lowAceSpades";
          return newComputerCards;
        });
  ...
  useEffect(() => {
    ...
    //Draw initial 2 cards for the user and computer
    drawComputerCardHandler();
    drawComputerCardHandler();
    drawUserCardHandler();
    drawUserCardHandler();
  }, []); //Since no dependencies only resolves when GameScreen is added to the UI
  ...
  }, [userScore]); //Dependent on user score and will activate when user score changes
